    ...
    for root, dirs, files in os.walk(DemultiplexFolder):
        for name in files:
            if 'fastq.gz' in name:
                ...
                if debug:
                    print( '/bin/mv ' + root + '/' + name + ' ' + root + '/' + RunId_short + '.' + name )

    for project in project_list:
        execute('/bin/mv ' + DemultiplexFolder + '/' + project.split('.')[0] + ' ' + DemultiplexFolder + '/' + RunId_short + '.'+ project, demultiplex_out_file)
        if debug:
            print('/bin/mv ' + DemultiplexFolder + '/' + project.split('.')[0] + ' ' + DemultiplexFolder + '/' + RunId_short + '.'+ project)
...
def qc(DemultiplexFolder, RunId_short, project_list, demultiplex_out_file):
    for project in project_list:
        ...
        execute('/usr/local/bin/fastqc -t 4 ' + project_folder + '/*fastq.gz' + ' > ' + DemultiplexFolder + '/demultiplex_log/04_fastqc.log', demultiplex_out_file)
        execute('/bin/cp ' + project_folder + '/*zip ' + project_folder + '/*html ' + DemultiplexFolder + '/' + RunId_short + '_QC', demultiplex_out_file)
...
def change_permission(folder_or_file, demultiplex_out_file):
    execute('/bin/chown -R seqtech:sambagroup ' + folder_or_file, demultiplex_out_file)
    execute('/bin/chmod -R g+rwX sambagroup ' + folder_or_file, demultiplex_out_file)

def main(RunId):

    DataFolder = '/data'
    RunLocation = DataFolder + '/' + 'rawdata'
    ...
    if checkComplete(RunFolder) == False:
        print (RunId + ' is not finished sequencing yet!!!')
        sys.exit()
    if createDirectory(DemultiplexFolder, RunId_short) ==  False:
    ...
    QC_tar_file = DataFolder + '/' + 'for_transfer/' + RunId_short + '_QC.tar'
